...
for i in $(seq 0 4); do
    for j in $(seq 1 2); do
        for k in $(seq 0 2); do
            ...
            MM_CMD="mm-delay ${DELAYS[j]} mm-link ${NTRACES[k]} ${DOWN_TRACE} --uplink-queue=dropbitrate_dequeue --uplink-queue-args=\"packets=250,log_file=${LOG_PREFIX}/octopus1_link.log,\""
            timeout 300 ../app/bbrserver ${PORT} > ${LOG_PREFIX}/octopus1_recv.log 2> /dev/null &
            $MM_CMD ./mm_aoi_send.sh ${PORT} ${VTRACES[i]} > ${LOG_PREFIX}/octopus1_send.log 2> /dev/null &
            ((PORT++))
            sleep 60
        done
    done
