...
for i in $(seq 0 4); do
    for j in $(seq 0 1); do
        for k in $(seq 2 2); do
            LOG_DIR="${HOME}/Research/Octopus-Data/svc-bottleneck/${VIDEOS[i]}/${NETWORKS[k]}_delay${DELAYS[j]}"
            ...
            $MM_CMD -- $MM2_CMD -- ./mm_svc_send.sh ${PORT} ${VTRACES[i]} > ${LOG_DIR}/octopus_send.log 2> /dev/null &
            ((PORT++))
            sleep 20
        done
    done
