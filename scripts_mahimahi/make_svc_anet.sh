...
for i in $(seq 0 4); do
    for j in $(seq 2 2); do
        for k in $(seq 2 2); do
            LOG_PREFIX="${HOME}/Research/Octopus-Data/svc-anet2/${VIDEOS[i]}/"
            MM_CMD="mm-delay ${DELAYS[j]} mm-link ${NTRACES[k]} ${DOWN_TRACE} --uplink-queue=dropactivenet --uplink-queue-args=\"packets=200,log_file=${LOG_PREFIX}/anet_200one_link.log,\""
            timeout 300 ../app/bbrserver ${PORT} > ${LOG_PREFIX}/anet_200one_recv.log 2> /dev/null &
            $MM_CMD ./mm_svc_send.sh ${PORT} ${VTRACES[i]} > ${LOG_PREFIX}/anet_200one_send.log 2> /dev/null &
            ((PORT++))
            sleep 20
        done
    done
