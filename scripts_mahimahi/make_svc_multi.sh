...
for i in $(seq 0 4); do
    for j in $(seq 0 1); do
        for k in $(seq 1 3); do
            LOG_DIR="${HOME}/Research/Octopus-Data/svc-multi/${VIDEOS[i]}/${NETWORKS[k]}_delay${DELAYS[j]}"
            ...
            timeout 300 ../app/bbrserver ${PORT1} > ${LOG_DIR}/octopus1_recv.log 2> /dev/null &
            timeout 300 ../app/bbrserver ${PORT2} > ${LOG_DIR}/octopus2_recv.log 2> /dev/null &
            $MM_CMD ./mm_svc_multi.sh ${PORT1} ${PORT2} ${VTRACES[i]} ${LOG_DIR} &
            ((PORT1++))
            ((PORT2++))
            sleep 60
        done
