...
for i in $(seq 0 0); do
    for j in $(seq 0 0); do
        for k in $(seq 0 0); do
            LOG_DIR="${HOME}/Research/Octopus-Data/volumetric/delay${DELAYS[j]}net${k}"
            ...
            timeout 300 ../app/ptcloud_server ${PORT} > ${LOG_DIR}/octopus_recv.log 2> /dev/null &
            $MM_CMD ./mm_ptcloud_send.sh ${PORT} ${VTRACES[i]} > ${LOG_DIR}/octopus_send.log 2> ${LOG_DIR}/octopus_send.error
            #$MM_CMD ./mm_ptcloud_send.sh ${PORT} ${VTRACES[i]} > ${LOG_DIR}/octopus_send.log 2> /dev/null &
            ((PORT++))
        done
    done
done
